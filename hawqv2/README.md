...
HAWQ-V2 does not observe final accuracy and therefore should not be treated as the only deployment selector when
the deployment objective is accuracy-constrained. In this paper repo, the intended independent workflow is:
...
3. The retained candidates are QAT-trained and evaluated using the normal ai8x flow.
4. ACE selects the final deployment configuration from the HAWQ frontier using QAT accuracy and model size.

This keeps the HAWQ route independent from the exhaustive QAT sweep. The exhaustive sweep can still be used later as a
reference comparison, but it is not an input to HAWQ-V2+ACE selection.
...
First export the HAWQ frontier candidates to a CSV that the QAT runner can consume:
...
Finally, use [select_ace_from_hawq.py](/Users/hamza/Documents/GitHub/Indoor-Environment-Quantization/hawqv2/tools/select_ace_from_hawq.py)
to apply ACE to the QAT results:
...
This means a model is not chosen by HAWQ alone. It is first retained by the HAWQ size-Omega frontier, then selected
by ACE after candidate-specific QAT evaluation.

To apply the workflow across multiple alpha values, pass multiple `--item` arguments:
...
python hawqv2/tools/export_hawq_candidates.py \
  ...
  --candidate-set frontier \
  --output hawqv2/hawqv2_runs/hawq_frontier_candidates.csv
```
...
Finally, export the union of HAWQ frontier candidates, run QAT on that CSV, and apply ACE to the QAT summary. The
candidate export command should include one `--item` per alpha:
...
python hawqv2/tools/export_hawq_candidates.py \
  ...
  --candidate-set frontier \
  --output hawqv2/hawqv2_runs/indoor_alpha_sweep_pareto/hawq_frontier_candidates.csv
```
...
The validation question is whether this HAWQ-frontier + QAT + ACE route selects the same deployment configuration as the
exhaustive QAT reference while requiring QAT only for the HAWQ-retained frontier.
