#!/usr/bin/env python3
"""
Mixed-Precision QAT Sweep for Indoor Environment (1D)
- Enumerates all 3^4 = 81 per-layer weight-bit configs across [8, 4, 2]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Mixed-Precision QAT sweep (81 configs)')
    parser.add_argument('num_seeds', nargs='?', type=int, default=5,
                        help='Number of seeds per configuration (default: 5)')
...
LOGS_DIR = OUTPUT_DIR / 'logs_mixed'
CHECKPOINTS_DIR = OUTPUT_DIR / 'checkpoints_mixed'
POLICY_DIR = REPO_ROOT / 'policies' / 'sweep'
...
def run_one(in_len, cfg_idx, cfg_bits, cfg_name, seed, policy_file, all_results):
    ...
    run_name = f"indoor_mixed_seed_{seed}__L{in_len}__{cfg_bits[0]}_{cfg_bits[1]}_{cfg_bits[2]}_{cfg_bits[3]}"
    ...
    try:
        ...
        write_rolling_results(all_results, OUTPUT_DIR, 'mixed_precision_sweep_results.csv', 'mixed_precision_sweep_summary.csv')
        return all_results
...
def write_policy_yaml(path, bits_tuple):
    ...
    policy = {
        'start_epoch': 8,
        'weight_bits': 8,  # default; per-layer overrides below
        'shift_quantile': 0.95,
...
CONFIGS = list(itertools.product(BITS, repeat=4))
SEEDS = [args.start_seed + i for i in range(args.num_seeds)]
SEEDS = sorted(SEEDS)
# Fixed preset input lengths 
PRESET_INPUT_LENGTHS = [101, 91, 81, 71, 61, 51, 41, 31, 21, 11, 5]

print("\n" + '=' * 80)
print(f"Mixed-Precision Sweep: {len(PRESET_INPUT_LENGTHS)} input lengths x {len(CONFIGS)} configs x {len(SEEDS)} seeds")
print('=' * 80)
print(f"Seeds: {SEEDS}")
...
for L_idx, in_len in enumerate(PRESET_INPUT_LENGTHS, 1):
    ...
    for cfg_idx, cfg_bits in enumerate(CONFIGS, 1):
        ...
        print("\n" + '-' * 80)
        print(f"Config {cfg_idx}/{len(CONFIGS)} | Length {in_len}: {cfg_name}")
        print('-' * 80)
        ...
        for s_idx in range(1, args.num_seeds + 1):
            ...
            print(f"Running seed {seed} for {cfg_name} at L={in_len}")
            all_results = run_one(in_len, cfg_idx, cfg_bits, cfg_name, seed, policy_file, all_results)


# Aggregate
df = pd.DataFrame(all_results)
df.to_csv(OUTPUT_DIR / 'mixed_precision_sweep_results.csv', index=False)
...
if len(df) > 0 and 'status' in df.columns:
    ...
    summary.sort_values(by='mean_acc', ascending=False, inplace=True)
    summary.to_csv(OUTPUT_DIR / 'mixed_precision_sweep_summary.csv', index=False)
else:
    summary = pd.DataFrame(columns=['input_length', 'config', 'runs', 'mean_acc', 'std_acc'])
    summary.to_csv(OUTPUT_DIR / 'mixed_precision_sweep_summary.csv', index=False)

print('\n' + '=' * 80)
print('Mixed-Precision Sweep Completed')
print('=' * 80)
print('Top configs:')
print(summary.head(10).to_string(index=False))
print(f"\nResults saved under: {OUTPUT_DIR}")
print('- mixed_precision_sweep_results.csv')
print('- mixed_precision_sweep_summary.csv')
