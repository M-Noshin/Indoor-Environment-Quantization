...
def main():
    ...
    if args.tblog and local_rank <= 0:  # not DistributedDataParallel or rank 0
        ...
        try:
            ...
            elif len(dimensions) == 2:  # For indoor environment: (101, 2)
                dummy_input = torch.randn((1, ) + dimensions)
            else:  # 1D input
                dummy_input = torch.randn((1, ) + dimensions[:-1])
            tflogger.tblogger.writer.add_graph(model.to('cpu'), (dummy_input, ), False)
        except Exception as e:
            msglogger.warning(f"Could not add graph to TensorBoard: {e}")
            msglogger.info("TensorBoard will still log metrics, just without the model graph.")
